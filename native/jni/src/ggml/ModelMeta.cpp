...
struct ModelMetadata loadModelMetadata(const std::string &modelPath) {
    ...
    // Standard GGUFs already contain a tokenizer and require no keyboardlm tokenizer payload.
    // Only read external tokenizer bytes when the metadata explicitly selects SentencePiece.
    if(result.ext_tokenizer_type == ExternalTokenizerType::SentencePiece) {
        const int kid = gguf_find_key(ctx_gguf, META_KEY_TOKENIZER_DATA_ARR);
        if (kid >= 0) {
            enum gguf_type ktype = gguf_get_kv_type(ctx_gguf, kid);
            if (ktype != GGUF_TYPE_ARRAY || gguf_get_arr_type(ctx_gguf, kid) != GGUF_TYPE_UINT8) {
                AKLOGE("key %s must be a byte array", META_KEY_TOKENIZER_DATA_ARR);
            } else {
    ...
    result.error = result.ext_tokenizer_type == ExternalTokenizerType::SentencePiece &&
            result.ext_tokenizer_data.empty();
    return result;
}
...
int writeModelMetadata(gguf_context *fctx, const ModelMetadata &metadata) {
    ...
    if(metadata.ext_tokenizer_type == ExternalTokenizerType::SentencePiece) {
